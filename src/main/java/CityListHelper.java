...
public class CityListHelper {
    public static HashMap<Integer, JSONObject> findEntrys(String filter, String fileName) throws IOException, ParseException {
        ...
        return null; //returning NULL when error occurred while loading the JSON
    }
    public static int listEntrys(HashMap<Integer,JSONObject> citys){
        ...
        return -1; //returning the value -1 because it isn't a valid ID so it's easy to trace back as an error that occurred in the selection
    }
}
